...
inline static void LCD_Send_Cmd(uint8_t cmd)
{
	CMD = cmd;
}
inline static void LCD_Send_Dat(uint8_t dat)
{
	DAT_C = dat;
}
inline static void LCD_Send_Dat_D(uint16_t dat)
{
	DAT = dat;
}
...
//{	
//	*(__IO uint8_t *)CMD = cmd;	
//	*(__IO uint8_t *)DAT = dat;
//}
...
inline static void LCD_Window(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	...
	LCD_Send_Dat(x1 >> 8);
	LCD_Send_Dat(x1 & 0xFF);
	LCD_Send_Dat(x2 >> 8);
	LCD_Send_Dat(x2 & 0xFF);
	LCD_Send_Cmd(LCD_PAGE_ADDR);
	LCD_Send_Dat(y1 >> 8);
	LCD_Send_Dat(y1 & 0xFF);
	LCD_Send_Dat(y2 >> 8);
	LCD_Send_Dat(y2 & 0xFF);
...
{
	LCD_Window(x, y, x, y);
	LCD_Send_Dat_D(H24_RGB565(1, color24));
}

void LCD_Rect_Fill(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t color24)//BILAL BILAL
{
	uint32_t i = 0;
	LCD_Window(x, y, x + w - 1, y + h - 1);
	for (i = 0; i < w * h; i++)
	{
		LCD_Send_Dat_D(H24_RGB565(0, color24));
	}
}
...
void LCD_Init(void)
{
	...
	LCD_RST_SET
	HAL_Delay (200);//HAL_Delay (100);
	LCD_RST_RESET
	HAL_Delay (200);//HAL_Delay (120);
	LCD_RST_SET
	HAL_Delay(200);
	...
	******************************/
	...
	LCD_Send_Dat(0x1D);//LCD_Send_Dat(0x28);//LCD_Send_Dat(0x1D);  // presceller(M=29) 
	LCD_Send_Dat(0x02);//LCD_Send_Dat(0x04);//LCD_Send_Dat(0x02);  //multiplier(N=2) 
	LCD_Send_Dat(0x04);  //on-off multiplier and presceller
	HAL_Delay(1);
	//3. Turn on the PLL 
	LCD_Send_Cmd(0xE0);
	LCD_Send_Dat(0x03);//LCD_Send_Dat(0x01);
	HAL_Delay(10); // Wait for 100us to let the PLL stable and read the PLL lock status bit. 
	LCD_Send_Cmd(0xE0);
	LCD_Send_Dat(0x03);
	HAL_Delay(12); 
	LCD_Send_Cmd(0x01);
	...
	//LCD_Send_Dat(0x01); // 5. Switch the clock source to PLL 
	//HAL_Delay(120);
	...
	********************/
	...
	//LCD_Send_Dat(0x03);     // Horizontal Width:  480 - 1 = 0x031F 
	//LCD_Send_Dat(0x1F);
	///LCD_Send_Dat(0x00);  /* 0x00 0x2d */   // 000 = режим RGB
	//--LCD_Send_Dat(0xAE);//LCD_Send_Dat(0xAE);
	LCD_Send_Dat(0x20);//LCD_Send_Dat(0x20);
	LCD_Send_Dat(0x00);//LCD_Send_Dat(0x01);     // Vertical Width :  800 -1 = 0x01DF
	LCD_Send_Dat((SSD_HOR_RESOLUTION-1)>>8);
	LCD_Send_Dat((uint8_t)(SSD_HOR_RESOLUTION - 1));     // Horizontal Width:  480 - 1 = 0x031F 
	LCD_Send_Dat((SSD_VER_RESOLUTION-1)>>8);
	LCD_Send_Dat((uint8_t)(SSD_VER_RESOLUTION-1));  /* 0x00 0x2d */   // 000 = RGB
	...
	LCD_Send_Dat((SSD_HT-1)>>8);
	LCD_Send_Dat((uint8_t)(SSD_HT-1));
	LCD_Send_Dat(SSD_HPS>>8);
	LCD_Send_Dat(SSD_HPS);
	LCD_Send_Dat(SSD_HOR_PULSE_WIDTH-1);
	LCD_Send_Dat(0x00);
	LCD_Send_Dat(0x00);
	LCD_Send_Dat(0x00);
	...
	LCD_Send_Dat((SSD_VT-1)>>8);
	LCD_Send_Dat((uint8_t)(SSD_VT-1));
	LCD_Send_Dat(SSD_VPS>>8);
	LCD_Send_Dat(SSD_VPS);
	LCD_Send_Dat(SSD_VER_FRONT_PORCH-1);
	LCD_Send_Dat(0x00);
	LCD_Send_Dat(0x00);
	...
	LCD_Send_Dat(0x03);//(0x03);     // 16 bit interface (565)
	LCD_Send_Cmd(LCD_DISPLAY_ON);     // display on 
	
	LCD_Send_Cmd(0xD0);     // display on 
	LCD_Send_Dat(0x00);
	...
	LCD_Send_Dat(0xFE);     // PWM duty cycle  (50%) ff
	LCD_Send_Dat(0x01);     // 0x09 = enable DBC, 0x01 = disable DBC  //on
	LCD_Send_Dat(0x00);	//4ÉèÖÃD
	LCD_Send_Dat(0x00);	//5ÉèÖÃE 
	//LCD_Send_Dat(0x01);	//6ÉèÖÃF 
	LCD_Send_Dat(0x00);	//6ÉèÖÃF 
	
	LCD_Send_Cmd(0xB8);	//ÉèÖÃGPIOÅäÖÃ
	LCD_Send_Dat(0x03);	//2¸öIO¿ÚÉèÖÃ³ÉÊä³ö
	LCD_Send_Dat(0x01);	//GPIOÊ¹ÓÃÕý³£µÄIO¹¦ÄÜ 
	LCD_Send_Cmd(0xBA);
	LCD_Send_Dat(0x01);	//GPIO[1:0]=01,¿ØÖÆLCD·½Ïò
	
	LCD_Bright(100);
	LCD_Send_Cmd(0x36);     //LCD_Bright(100); set address_mode
	LCD_Send_Dat((0<<3)|(0<<7)|(0<<6)|(0<<5));
    ...
    for(i=0;i<LCD_WIDTH;i++)
	 {
	  for (j=0;j<LCD_HEIGHT;j++)
	   	{
				LCD_Send_Dat_D(H24_RGB565(1,Color));
	    }
...
{
	LCD_Send_Cmd(0xBE);  // PWM configuration 
	LCD_Send_Dat(0x05);//LCD_Send_Dat(0x08);     // set PWM signal frequency to 170Hz when PLL frequency is 100MHz 
	LCD_Send_Dat(bright*2.55);   // PWM duty cycle  
	LCD_Send_Dat(0x01);
	LCD_Send_Dat(0xFF);
	LCD_Send_Dat(0x00);
	LCD_Send_Dat(0x00);
...
void SSD1963_printImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data, uint32_t size)
{
	uint32_t n = size;
	LCD_Window(x, y, x + w-1, y + h-1);
	for(uint32_t i=0; i < n ; i += 2)
	{
		//uint32_t nData = ( (data[i] >> 16) | (data[i+1] >> 8) | data[i+2] ); 
		//uint32_t nData = data[i]; 
		//nData <<= 8; nData |= data[i+1];nData <<= 8; nData |= data[i+3];
		uint16_t nData = data[i]; 
		nData <<= 8; nData |= data[i+1];//nData <<= 8; nData |= data[i+3];
		LCD_Send_Dat_D(nData);
		//LCD_Send_Dat(H24_RGB565(1, nData));
	}
}
void SSD1963_printImage16Bit(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t *data, uint32_t size)
{
	uint32_t n = size;
	LCD_Window(x, y, x + w-1, y + h-1);
	for(uint32_t i=0; i < n ; i += 1)
	{
		...
		//uint16_t nData = data[i]; 
		//nData <<= 8; nData |= data[i+1];//nData <<= 8; nData |= data[i+3];
		LCD_Send_Dat_D(data[i]);
		//LCD_Send_Dat(H24_RGB565(1, nData));
	}
}
void ssd196xMainScreen()
{
	
	SSD1963_printImage16Bit(99, 49, 100, 100, epd_bitmap_Gallary, sizeof(epd_bitmap_Gallary));
	//LCD_Rect_Round(98, 47, 100, 100, 20, 8, RED);
	//LCD_Circle(148, 98, 50, 0, 5, RED);
	SSD1963_printImage16Bit(349, 49, 100, 100, epd_bitmap_Settings, sizeof(epd_bitmap_Settings));
	SSD1963_printImage16Bit(599, 49, 100, 100, epd_bitmap_Test_Pattern, sizeof(epd_bitmap_Test_Pattern));
	SSD1963_printImage16Bit(99, 199, 100, 100, epd_bitmap_Color_Selector, sizeof(epd_bitmap_Color_Selector));
	SSD1963_printImage16Bit(349, 199, 100, 100, epd_bitmap_Hooks, sizeof(epd_bitmap_Hooks));
	
	SSD1963_printImage16Bit(599, 199, 100, 100, epd_bitmap_Pick_Lock, sizeof(epd_bitmap_Pick_Lock));
	SSD1963_printImage16Bit(99, 349, 100, 100, epd_bitmap_Restore, sizeof(epd_bitmap_Restore));
	SSD1963_printImage16Bit(349, 349, 100, 100, epd_bitmap_Contact, sizeof(epd_bitmap_Contact));
	SSD1963_printImage16Bit(599, 349, 100, 100, epd_bitmap_Future, sizeof(epd_bitmap_Future));

	return;
}
void selectIcon(uint8_t iconSelected)
{
	static uint8_t prevIconSelected = 0; 
	switch(prevIconSelected)
	{
		case 1:
			LCD_Rect_Round(98, 47, 100, 100, 20, 8, BLACK);
			break;
		case 2:
			LCD_Rect_Round(348, 47, 100, 100, 20, 8, BLACK);
			break;
		case 3:
			LCD_Rect_Round(598, 47, 100, 100, 20, 8, BLACK);
			break;
		case 4:
			LCD_Rect_Round(98, 197, 100, 100, 20, 8, BLACK);
			break;
		case 5:
			LCD_Rect_Round(348, 197, 100, 100, 20, 8, BLACK);
			break;
		case 6:
			LCD_Rect_Round(598, 197, 100, 100, 20, 8, BLACK);
			break;
		case 7:
			LCD_Rect_Round(98, 347, 100, 100, 20, 8, BLACK);
			break;
		case 8:
			LCD_Rect_Round(348, 347, 100, 100, 20, 8, BLACK);
			break;
		case 9:
			LCD_Rect_Round(598, 347, 100, 100, 20, 8, BLACK);
			break;
		default:
			break;
	}
	switch(iconSelected)
	{
		case 1:
			...
			break;
		case 2:
			LCD_Rect_Round(348, 47, 100, 100, 20, 8, RED);
			prevIconSelected = iconSelected;
			break;
		case 3:
			LCD_Rect_Round(598, 47, 100, 100, 20, 8, RED);
			prevIconSelected = iconSelected;
			break;
		case 4:
			LCD_Rect_Round(98, 197, 100, 100, 20, 8, RED);
			prevIconSelected = iconSelected;
			break;
		case 5:
			LCD_Rect_Round(348, 197, 100, 100, 20, 8, RED);
			prevIconSelected = iconSelected;
			break;
		case 6:
			LCD_Rect_Round(598, 197, 100, 100, 20, 8, RED);
			prevIconSelected = iconSelected;
			break;
		case 7:
			LCD_Rect_Round(98, 347, 100, 100, 20, 8, RED);
			prevIconSelected = iconSelected;
			break;
		case 8:
			LCD_Rect_Round(348, 347, 100, 100, 20, 8, RED);
			prevIconSelected = iconSelected;
			break;
		case 9:
			LCD_Rect_Round(598, 347, 100, 100, 20, 8, RED);
			prevIconSelected = iconSelected;
			break;
		default:
			break;
