...
//2. Write data to LCD
void ILI9341_SendData_16(uint16_t data)
{
...
void ILI9341_Init(void)
 {
   ...
   ILI9341_SendData   (0x02);//(0x55); // 16bit /pixel
   ...
   ILI9341_SendData   (0x00); // x0_HIGH---0
   ILI9341_SendData   (0x00); // x0_LOW----0
   ILI9341_SendData   (0x01); // x1_HIGH---240
   ILI9341_SendData   (0xE0); // x1_LOW----240
   ILI9341_SendCommand (ILI9341_PAGE_ADDR); // page address set
   ILI9341_SendData   (0x00); // y0_HIGH---0
   ILI9341_SendData   (0x00); // y0_LOW----0
   ILI9341_SendData   (0x03); // y1_HIGH---320
   ILI9341_SendData   (0x20); // y1_LOW----320
...
void ILI9341_Fill(uint16_t color) {
	...
	while (n) {
			n--;
				ILI9341_SendData_16(color>>16);
		ILI9341_SendData_16(color & 0xffff);
				//ILI9341_SendData_16(0x0000);
       /*ILI9341_SendData(color>>8);
				ILI9341_SendData(color&0xff);*/
	}
}
